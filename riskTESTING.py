...
class Player:
    ...
    def place_army(self, territory):
        if self.armies_to_place > 0:
            ...
            print(f"{self.name} placed an army on {territory.name}. Troops left now to place: {self.armies_to_place}")
        else:
...
class Game:
    ...
    def draw_status_bar(self): # draws the status bar (duh) that displays who's turn it is 
        if self.phase == Phase.SETUP:  # Only display the status bar during the SETUP phase
            ...
            if self.current_player: # Render the current player's name
                text = self.font.render(f"Current Player: {self.current_player.name}", True, self.current_player.color)
                text_rect = text.get_rect(center=self.bar_rect.center)  # Center the text in the status bar
                self.screen.blit(text, text_rect)
        
        if self.phase == Phase.PLACING_ARMY:
            pygame.draw.rect(self.screen, self.WHITE, self.bar_rect)
            if self.current_player: # Render the current player's name
                text = self.font.render(f"Place down {self.current_player.armies_to_place} armies {self.current_player.name}", True, self.current_player.color)
                text_rect = text.get_rect(center=self.bar_rect.center)  # Center the text in the status bar
                self.screen.blit(text, text_rect)
    ...
    def initial_next_turn(self): # allows players to take turns placing troops and transition to next phase of game
        ...
        if all(player.initial_armies_to_place == 0 for player in self.players):
            self.phase = Phase.PLACING_ARMY  # Transition to main game phase
    ...
    def setup_initial_armies(self):
        """Automatically place armies for the current player during setup."""
        print(f"Setting up armies for {self.current_player.name}")

        # Place armies in any of the current player's territories
        while self.current_player.initial_armies_to_place > 0:
            available_territories = self.current_player.territories  # All territories owned by the player
            if available_territories:
                ...
                print(f"{self.current_player.name} has no territories left.")
                break  # In case of an unexpected situation
    ...
    def run(self):
        ...
        while running:
                ...
                for event in pygame.event.get():
                    ...
                    if self.phase == Phase.SETUP:  # Automatic setup for testing
                        self.setup_initial_armies()  # Call to the new function for automatic setup
                        ...
                        if event.type == pygame.MOUSEBUTTONDOWN:
                            ...
                            for territory in territories:
                                if territory.is_clicked(mouse_pos):
                                    if territory in self.selected_territories:
                                        self.selected_territories.remove(territory)  # Deselect if already selected
                                        print(f"Deselected: {territory.name}")
                                    else:
                                        if len(self.selected_territories) < 2:
                                            self.selected_territories.append(territory)  # Select if less than 2 items are selected
                                            print(f"Selected: {territory.name}")
                                        elif len(self.selected_territories) == 2:
                                            self.selected_territories.pop(0)  # Deselect the first one if already two are selected
                                            self.selected_territories.append(territory)
                                            print(f"Deselected: {self.selected_territories[0].name} and Selected: {territory.name}")
